...
function callClaude(prompt, reports, callback) {
  ...
  const options = {
    ...
    method: 'POST',
    headers,
    timeout: 330000 // 5.5 minute timeout — headroom for max_tokens 20000 + PDF processing
  };
...
const server = http.createServer((req, res) => {
  ...
  if (req.method === 'GET' && req.url === '/health') {
    ...
    res.end(JSON.stringify({
      status: 'ok',
      version: 'v4-macro-breakdown-2026-07-05',
      keySet: !!API_KEY,
...
server.timeout = 350000; // ~5.8 minutes — must exceed the outbound Claude request timeout above
server.listen(PORT, () => {
  console.log('=== SERVER VERSION: v4-macro-breakdown-2026-07-05 ===');
